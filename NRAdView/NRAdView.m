...
#import "NRAdView.h"
#import "NRAdCollectionViewLayout.h"
...
@interface NRAdView()<NRCollectionViewFlowLayoutDelegate>
{
    NRAdViewBuilder *_builder;
    ///因为当用户滑动的时候，轮播不应该继续，所以会被停掉，但是当滑动结束，会根据是否开启了轮播而重新开启
    BOOL _isPlaying; //真实是否开启了播放的状态
    BOOL _expctedToPlay;//用户启动的play
    NRAdCollectionViewLayout *_layout;
}
/**
 *   计时器
 */
...
@implementation NRAdViewBuilder
@end

@interface NRAdView()<UICollectionViewDelegate,UICollectionViewDataSource>
/**
 *   collection
 */
...
@implementation NRAdView

-(instancetype)initWithBuilder:(void (^) (NRAdViewBuilder *builder))builderBlock{
    if (self = [super init]) {
        NRAdViewBuilder *builder = [NRAdViewBuilder new];
 ...
 *  @brief 初始化
 */
-(void)setUpWithBuilder:(NRAdViewBuilder *)builder{
    _builder = builder;
    ...
    NRAdCollectionViewLayout *layout = [NRAdCollectionViewLayout new];
    ...
    self.collectionView = [[UICollectionView alloc]initWithFrame:(CGRect){0,0,self.frame.size.width,self.frame.size.height} collectionViewLayout:layout];
...
-(void)_playNextAd{
    ...
    if(_builder.autoScrollDirection==NRAdViewScrollDirectionRight || _builder.autoScrollDirection==NRAdViewScrollDirectionBotom){
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    UICollectionViewCell<NRAdViewRenderDelegate> *cell = [collectionView dequeueReusableCellWithReuseIdentifier:SC_AD_CELL_IDENTIFIER forIndexPath:indexPath];
    [cell setModel:model];
    return cell;
}
